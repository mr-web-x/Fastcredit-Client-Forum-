...
class AdminServiceServer {
  ...
  // === DASHBOARD И СТАТИСТИКА (всё через serverApiClient) ===

  async getDashboard() {
    try {
      return await this.client.get("/admin");
    } catch (e) {
      if (process.env.NODE_ENV === "development") {
        console.error("[AdminServiceServer.getDashboard]", e.message);
      }
      throw e;
    }
  }
  ...
  async exportData(exportConfig) {
    // На сервере тоже можно получить blob/stream — зависит от бэка.
  ...
  // ——— Дополнительно: сохранил «серверные» алиасы из исходника — можно выкинуть, но пусть будут совместимы:
  async getDashboardServer() {
    try {
      return await this.getDashboard();
    } catch (e) {
      if (process.env.NODE_ENV === "development")
        console.error("[AdminServiceServer.getDashboardServer]", e.message);
      return null;
    }
  }

  async getForumStatisticsServer(params = {}) {
    try {
      return await this.getForumStatistics(params);
    } catch (e) {
      if (process.env.NODE_ENV === "development")
        console.error(
          "[AdminServiceServer.getForumStatisticsServer]",
          e.message
        );
      return null;
    }
...
export default new AdminServiceServer();
