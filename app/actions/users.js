...
/**
 * Получение истории изменения ролей пользователя (только для админов)
 */
export async function getUserRoleHistoryAction(userId, params = {}) {
  try {
    const user = await getServerUser();
    if (!user || user.role !== "admin") {
      return {
    ...
    const result = await usersService.getUserRoleHistory(userId, {
      page,
      limit,
    });
    ...
    console.error("❌ Failed to get user role history:", error);
    return {
      success: false,
      error: "Nepodarilo sa načítať históriu rolí",
    };
  }
}
 ...
 */
export async function getUsersStatisticsAction() {
  try {
    const user = await getServerUser();
    if (!user || user.role !== "admin") {
      return {
    ...
    console.error("❌ Failed to get users statistics:", error);
    return {
      success: false,
      error: "Nepodarilo sa načítať štatistiky používateľov",
    };
  }
}
